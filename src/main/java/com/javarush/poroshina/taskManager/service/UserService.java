...
public class UserService {
    ...
    private List<UserResponseDto> toResponseList(List<User> users) {
        return users.stream()
                .map(this::toResponse)
                .collect(Collectors.toList());
    }
    ...
    public List<UserResponseDto> getUsersWithAuthoredTasks() {
        return toResponseList(userRepository.findUsersWithAuthoredTasks());
    }

    @Transactional(readOnly = true)
    public UserResponseDto getUserByAuthoredTaskId(Long taskId) {
        User user = userRepository.findUserByAuthoredTaskId(taskId)
                .orElseThrow(() -> new UserNotFoundException(AppConstants.AUTHOR_NOT_FOUND_FOR_TASK_MESSAGE + taskId));
    ...
    public List<UserResponseDto> getUsersWithoutAuthoredTasks() {
        return toResponseList(userRepository.findUsersWithoutAuthoredTasks());
    }

    @Transactional(readOnly = true)
    public List<UserResponseDto> getUsersWithExecutedTasks() {
        return toResponseList(userRepository.findUsersWithExecutedTasks());
    }

    @Transactional(readOnly = true)
    public UserResponseDto getUserByExecutedTaskId(Long taskId) {
        User user = userRepository.findUserByExecutedTaskId(taskId)
                .orElseThrow(() -> new UserNotFoundException(AppConstants.EXECUTOR_NOT_FOUND_FOR_TASK_MESSAGE + taskId));
    ...
    public List<UserResponseDto> getUsersWithoutExecutedTasks() {
        return toResponseList(userRepository.findUsersWithoutExecutedTasks());
    }

    @Transactional(readOnly = true)
    public List<UserResponseDto> getUsersWithTasksByStatus(TaskStatus status) {
        return toResponseList(userRepository.findUsersWithExecutedTasksByStatus(status));
    }

    @Transactional(readOnly = true)
    public List<UserResponseDto> searchUsersByUsername(String username) {
        return toResponseList(userRepository.findByUsernameContainingIgnoreCase(username));
    }
}
